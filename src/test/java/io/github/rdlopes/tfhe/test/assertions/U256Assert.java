...
public class U256Assert extends AbstractAssert<U256Assert, U256> {
  ...
  public U256Assert isGreaterThan(U256 expected) {
    ...
    if (actualValue.compareTo(expectedValue) <= 0) {
      failWithMessage("Expected U256 to be greater than <%s> but was <%s>", expectedValue, actualValue);
    }
    return this;
  }

  public U256Assert isGreaterThan(BigInteger expected) {
    isNotNull();
    if (expected == null) {
      failWithMessage("Expected BigInteger should not be null");
    }
    ...
    if (actualValue.compareTo(expected) <= 0) {
      failWithMessage("Expected U256 to be greater than <%s> but was <%s>", expected, actualValue);
    }
    return this;
  }

  public U256Assert isGreaterThan(long expected) {
    if (expected < 0) {
      failWithMessage("Expected long should not be negative for U256 comparison");
    }
  ...
  public U256Assert isGreaterThan(String expected) {
    if (expected == null) {
      failWithMessage("Expected String should not be null");
    }
    BigInteger expectedBigInt = new BigInteger(expected);
