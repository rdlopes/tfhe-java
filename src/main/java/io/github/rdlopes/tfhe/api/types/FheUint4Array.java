...
// @formatter:off
public class FheUint4Array extends NativeArray implements FheArray<FheUint4, FheUint4Array> {
...
public FheUint4 sum(){
  ...
  @Override
  public FheUint4Array add(FheUint4Array other) {
    if (this.getSize() != other.getSize()) {
      throw new IllegalArgumentException("Array sizes must match");
    }
    java.util.List<FheUint4> thisElements = this.getElements();
    java.util.List<FheUint4> otherElements = other.getElements();
    java.util.List<FheUint4> result = new java.util.ArrayList<>();
    for (int i = 0; i < thisElements.size(); i++) {
      result.add(thisElements.get(i).add(otherElements.get(i)));
    }
    return new FheUint4Array(result);
  }

  @Override
  public FheUint4Array subtract(FheUint4Array other) {
    if (this.getSize() != other.getSize()) {
      throw new IllegalArgumentException("Array sizes must match");
    }
    java.util.List<FheUint4> thisElements = this.getElements();
