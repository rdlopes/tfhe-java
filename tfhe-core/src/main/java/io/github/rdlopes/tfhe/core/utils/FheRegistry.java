...
/// stored in per-type [io.github.rdlopes.tfhe.core.ffm.FheTypeHandles] records
...
public final class FheRegistry {
  ...
  public static FheOps.UnaryOp getCastOp(Class<?> sourceClass, Class<?> targetClass) {
    FheOps.UnaryOp op = CAST_OPS.computeIfAbsent(
      ...
      pair -> resolveUnaryOp(
        FheRegistry.nativeType(pair.first().getSimpleName())
          + "_cast_into_"
          + FheRegistry.nativeType(pair.second().getSimpleName()))
        .orElse(CAST_ABSENT));
  ...
  public static FheOps.ListGetOp getCompressedGetOp(Class<?> targetClass) {
    FheOps.ListGetOp op = COMPRESSED_GET_OPS.computeIfAbsent(
      targetClass,
      c -> resolveListGetOp(
        "compressed_ciphertext_list_get_" + FheRegistry.nativeType(c.getSimpleName()))
  ...
  public static FheOps.ListGetOp getCompactGetOp(Class<?> targetClass) {
    FheOps.ListGetOp op = COMPACT_GET_OPS.computeIfAbsent(
      targetClass,
      c -> resolveListGetOp(
        "compact_ciphertext_list_expander_get_" + FheRegistry.nativeType(c.getSimpleName()))
        .orElse(LIST_GET_ABSENT));
    return op == LIST_GET_ABSENT ? null : op;
  }
  ...
  public static FheOps.TryDecryptPrimitiveOp getTryDecryptPrimitiveOp(Class<?> clazz) {
    FheOps.TryDecryptPrimitiveOp op = TRY_DECRYPT_PRIM.computeIfAbsent(
      clazz,
      c -> resolveUnaryOp(
        FheRegistry.nativeType(c.getSimpleName()) + "_try_decrypt_trivial")
  ...
  /// Converts a CamelCase type name to its native snake_case equivalent.
  /// Example: "FheUint8" → "fhe_uint8", "FheBool" → "fhe_bool".
  static String nativeType(String typeName) {
